...
def get_data():
	...
	if plot is not None:
		...
		plot = plots.create_scatter(plot_area, plot_data)
...
@main.route('/d3_data', methods = ['GET', 'POST'])
def d3_data():
	area_name = request.args.get("area_name")
	...
	plot_data = data.stats_ams.drop('area_code', axis=1)
	plot_data = plot_data.to_json(orient='records')

	meta_data = data.stats_ams_meta.to_json(orient='records')
	return render_template("d3_data.html", data=plot_data, meta_data=meta_data,
		x_variables=data.all_var_types, area_names=data.area_names, selected_area_name=area_name)
